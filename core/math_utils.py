...
def quaternion_slerp(q1, q2, t):
    ...
    if dot > 0.9995:
        ...
        result = (1 - t) * q1 + t * q2
        return normalize_quarternion(result)
...
def kinetic_energy(body):
    Ttrans = 0.5 * body.mass * np.dot(body.linear_velocity, body.linear_velocity)
    Iw = body.get_inertia_world(); omega = body.angular_velocity
    Trot = 0.5 * np.dot(omega, np.dot(Iw, omega))
    return Ttrans + Trot
